...
COPY . .
RUN apt update -y
RUN apt install -y python3-tk python3.10-venv libcairo2-dev \
    libcudnn8=8.7.0.84-1+cuda11.8 libcudnn8-dev=8.7.0.84-1+cuda11.8 --allow-change-held-packages
RUN DEBIAN_FRONTEND=noninteractive TZ=Asia/Tokyo apt install -y tzdata && \
...
RUN pip install fastapi tensorflow-gpu==2.8.0 uvicorn xformers==0.0.21 bitsandbytes==0.41.1 datasets toml Pillow python-dotenv runpod accelerate==0.12.0
